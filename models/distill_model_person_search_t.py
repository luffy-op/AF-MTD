...
# APTM
from models.aptm.model_retrieval import APTM_Retrieval
from ruamel.yaml import YAML

yaml_aptm = YAML(typ='rt')
with open('configs/aptm/Retrieval_cuhk.yaml', 'r') as file:
    config_aptm = yaml_aptm.load(file)
...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        # self.visual_encoder = VisionTransformer(
        #     img_size=config['image_res'], patch_size=16, embed_dim=192, depth=12, num_heads=3,
        #     mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), )
        ...
        # if teacher == "clip_vitb":
        #     # encoders.append(teacher_models[teacher](config, num_classes))
        #         encoder = teacher_models[teacher](config, num_classes)
        #         if path_to_pretrained_weights is not None:
        #             checkpoint = torch.load(os.path.join(path_to_pretrained_weights, "checkpoint_best.pth"))
        #             encoder.load_state_dict(checkpoint['model'])  # 加载模型权重
        #         encoders.append(encoder)
        
        # teacher1:   TBPS-CLIP
        self.model_t1 = clip_vitb(config=config_tbpsclip, num_classes=tbpsclip_numclasses)
        checkpoint = torch.load(os.path.join("/workspace/zl/t2i-reid/TBPS-CLIP-master/ckpts/baseline_224_224/CUHK-PEDES", "checkpoint_best.pth"))
        self.model_t1.load_state_dict(checkpoint['model'])  # 加载模型权重
    ...
    def forward(self, image1, image2, text1, text2, text_tbpsclip, alpha, idx, replace):
        ...
        with torch.no_grad():
            # tch1_image_embeds, tch1_text_output = self.model_t1()
            ...
            # tch1_text = tokenize(text_tbpsclip, context_length=77).to('cuda')
            # print('text1.input_ids: ', text1.input_ids.shape)
            tch1_text_feat = F.normalize(self.model_t1.encode_text(text_tbpsclip), dim=-1)

            # tch2_image_embed, _ = self.model_t2.get_vision_embeds(image1)
            # tch2_image_feat = self.model_t2.vision_proj(tch2_image_embed[:, 0, :])
            # tch2_image_feat = F.normalize(tch2_image_embed[:, 0, :], dim=-1)

            # tch2_text_embed = self.model_t2.get_text_embeds(text1.input_ids, text1.attention_mask)
            # tch2_text_feat = self.model_t2.text_proj(tch2_text_embed[:, 0, :])
            # tch2_text_feat = F.normalize(tch2_text_embed[:, 0, :], dim=-1)

            

        # stu_t1_image_feat = F.normalize(self.vision_proj_t1(image_embeds[:, 0, :]), dim=-1)
        # stu_t1_text_feat = F.normalize(self.text_proj_t1(text_output.last_hidden_state[:, 0, :]), dim=-1)
        ...
        # loss_distill = self.loss_fn_distill(stu_t1_image_feat, stu_t1_text_feat, tch1_image_feat, tch1_text_feat)
        ...
        # return loss_cl, loss_pitm, loss_mlm, loss_prd, loss_mrtd, loss_mae, loss_attribute, loss_distill
