...
class ALBEF(nn.Module):
    def __init__(
        ...
        # self.visual_decoder = CrossTransformer(768, 8, 96, depth=4, context_dim=768)
        ...
        # self.text_encoder_cross = BertForMaskedLM.from_pretrained(
        #     "/workspace/zl/t2i-reid/APTM/data/bert-base-uncased", config=bert_config
        # )
        self.text_encoder = AutoModel.from_pretrained(text_encoder)  # tttttttt
        ...
        # self.text_encoder_cross_m = BertForMaskedLM.from_pretrained(
        #     "/workspace/zl/t2i-reid/APTM/data/bert-base-uncased", config=bert_config
        # )
        ...
        # teacher1:   TBPS-CLIP
        # self.model_t1 = clip_vitb(
        #     config=config_tbpsclip, num_classes=tbpsclip_numclasses
        # )
        # checkpoint_t1 = torch.load(
        #     os.path.join(
        #         "/workspace/zl/t2i-reid/TBPS-CLIP-master/ckpts/baseline_224_224/CUHK-PEDES",
        #         "checkpoint_best.pth",
        #     )
        # )
    ...
    def forward(self, image1, image2, text1, text2, text_tbpsclip, alpha, idx, replace):
        ...
        text_output = self.text_encoder(
            text2.input_ids, attention_mask=text2.attention_mask, return_dict=True
        )
        ...
        with torch.no_grad():
            ...
            sim_i2i_targets = (
                alpha * F.softmax(sim_i2i_m, dim=1) + (1 - alpha) * sim_targets
            )
            sim_t2t_targets = (
                alpha * F.softmax(sim_t2t_m, dim=1) + (1 - alpha) * sim_targets
            )
        ...
        # # T2
        # text_output_t2 = self.encoders_t[1].text_encoder.bert(
        #     text2.input_ids, attention_mask=text2.attention_mask, mode="text"
        # )
        # out_img_t.append(
        #     self.encoders_t[1].vision_proj(
        #         self.encoders_t[1].visual_encoder(image1)[:, 0, :]
        #     )
        # )
        # out_text_t.append(text_output_t2.last_hidden_state[:, 0, :])

        # out_img_t = nn.functional.normalize(out_img_t, p=2, dim=-1)
        # out_text_t = nn.functional.normalize(out_text_t, p=2, dim=-1)
        ...
        with torch.no_grad():
            ...
            tch1_text_feat = F.normalize(
                self.model_t1.encode_text(text_tbpsclip), dim=-1
            )
        ...
        loss_i2i = -torch.sum(
            F.log_softmax(sim_i2i, dim=1) * sim_i2i_targets, dim=1
        ).mean()
        loss_t2t = -torch.sum(
            F.log_softmax(sim_t2t, dim=1) * sim_t2t_targets, dim=1
        ).mean()
        loss_cl = (loss_i2t + loss_t2i + loss_i2i + loss_t2t) / 4

        loss_distill = self.loss_fn_distill(
            image_feat, text_feat, t1_proj_image_feat, t1_proj_text_feat
        )
        ...
        # import sys
        # sys.exit()
        output_pos = self.text_encoder_cross.bert(
            text2.input_ids,
            attention_mask=text2.attention_mask,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=image_atts,
        ...
        output_neg_cross = self.text_encoder_cross.bert(
            text_inputs_ids_all,
        ...
        input_ids, labels = self.mask(
            input_ids,
            self.text_encoder_cross.config.vocab_size,
            targets=labels,
            probability_matrix=probability_matrix,
        )
        with torch.no_grad():
            logits_m = self.text_encoder_cross_m(
                input_ids,
        ...
        mlm_output = self.text_encoder_cross(
            input_ids,
            attention_mask=text1.attention_mask,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=image_atts,
    ...
    def mrtd_mask_modeling(
        self, mrtd_input_ids, ori_input_ids, attention_mask, weights
    ):
