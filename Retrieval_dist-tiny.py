...
from models.distill_model_person_search_tiny import ALBEF
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    metric_logger.add_meter('loss_distill', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i, (image1, image2, text1, text2, idx, replace) in enumerate(
        ...
        loss_cl, loss_pitm, loss_mlm, loss_distill = model(image1, image2, text_input1, text_input2, text_tbpsclip, 
...
def main(args, config):
    ...
    model = ALBEF(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, tbpsclip_numclasses=num_classes)
