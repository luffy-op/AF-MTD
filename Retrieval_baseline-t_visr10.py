...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image1, image2, text1, text2, idx, replace) in enumerate(
        ...
        loss_cl, loss_pitm, loss_mlm = model(image1, image2, text_input1, text_input2,
                                                                  alpha=alpha, idx=idx, replace=replace)
        loss = 0.
        for j, los in enumerate((loss_cl, loss_pitm, loss_mlm)):
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.text_encoder_cross.bert(text_inputs_ids[start + i].repeat(config['k_test'], 1),
        ...
        # Save top 10 images and query text for each text query
        if i < 10:
            top10_idx = topk_idx[:10].cpu().numpy()
            top10_images = [data_loader.dataset.image[idx] for idx in top10_idx]
            query_text = texts[start + i]
            # print('query_text:', query_text)
            # print('top10_idx: ', top10_idx)
            # print('top10_images: ', top10_images)
            # import sys
            # sys.exit()
            with open(f"output/cuhk-pedes/visr10_base_t/top10_query_{start + i}.txt", "w") as f:
                f.write(query_text + "\n")
                f.write("Image IDs:\n")
                for name in top10_images:
                    f.write(f"{name}\n")
            for j, img_path in enumerate(top10_images):
                absoulte_path = os.path.join('/workspace/zl/t2i-reid/datasets/CUHK-PEDES/imgs', img_path)
                img = Image.open(absoulte_path)
                img.save(f"output/cuhk-pedes/visr10_base_t/top10_query_{start + i}_img_{j}.png")
