...
from models.distill_single_model_person_search_tiny import ALBEF
...
from text_utils.tokenizer import tokenize as tokenize_tbpsclip

# # 选择轻量化的 BERT 变体，例如 DistilBERT
# model_name = "distilbert-base-uncased"  # 可替换为 "huawei-noah/TinyBERT_General_4L_312D" 或 "microsoft/MiniLM-L6-H384-uncased"
...
def train(
    ...
    metric_logger.add_meter(
        "loss_cl", utils.SmoothedValue(window_size=1, fmt="{value:.4f}")
    )
    metric_logger.add_meter(
        "loss_pitm", utils.SmoothedValue(window_size=1, fmt="{value:.4f}")
    )
    metric_logger.add_meter(
        "loss_mlm", utils.SmoothedValue(window_size=1, fmt="{value:.4f}")
    )
    metric_logger.add_meter(
        "loss_distill", utils.SmoothedValue(window_size=1, fmt="{value:.4f}")
    )
    ...
    for i, (image1, image2, text1, text2, idx, replace) in enumerate(
        ...
        text_tbpsclip = tokenize_tbpsclip(text1, context_length=77).to(device)
        ...
        loss_cl, loss_pitm, loss_mlm, loss_distill = model(
            image1,
            image2,
            text_input1,
            text_input2,
        ...
        loss = 0.0
        for j, los in enumerate((loss_cl, loss_pitm, loss_mlm, loss_distill)):
    ...
    for chunk in doc.noun_chunks:
        ...
        for tok in chunk:
            if tok.pos_ == "NOUN":
                ...
                while (
                    char_count <= tok.idx
                    and tok.idx <= char_count + len(text_words[idx_words])
                ) == False:
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for image, img_id in data_loader:
        ...
        # image_feat_db = model.dbert2bert_proj(image_feat)
        image_feat = model.dbert2bert_proj(image_feat)
        image_feat = model.norm_db(image_feat)

        image_embed = model.vision_proj(image_feat[:, 0, :])
        image_embed = F.normalize(image_embed, dim=-1)
        image_feats.append(image_feat.cpu())
        # image_feats_db.append(image_feat_db.cpu())
        image_embeds.append(image_embed)

    image_feats = torch.cat(image_feats, dim=0)
    # image_feats_db = torch.cat(image_feats_db, dim=0)
    ...
    for i, sims in enumerate(
        ...
        output = model.text_encoder_cross.bert(
            text_inputs_ids[start + i].repeat(config["k_test"], 1),
...
def main(args, config):
    ...
    # tokenizer = BertTokenizer.from_pretrained('/workspace/zl/t2i-reid/APTM/data/bert-base-uncased')
    ...
    model = ALBEF(
        ...
        tokenizer=tokenizer,
        tbpsclip_numclasses=num_classes,
    )
        ...
        if epoch >= config["eval_epoch"] or args.evaluate:
            ...
            if utils.is_main_process():
                test_result = itm_eval(
                    score_test_t2i,
                    test_dataset.img2person,
                    test_dataset.txt2person,
                    args.eval_mAP,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_mAP", action="store_true", help="whether to evaluate mAP"
    )
