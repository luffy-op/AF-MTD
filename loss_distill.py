...
class MultiTeacherDistillLoss(nn.Module):
    ...
    def forward(self, stu1, stu2, tch1, tch2):
        ...
        # print('stu1.shpae: ', stu1.shape) # 8*512
        # print('stu2.shpae: ', stu2.shape)
        # print('tch1.shpae: ', tch1.shape)
        # print('tch2.shpae: ', tch2.shape)
        stu1 = nn.functional.normalize(stu1, p=2, dim=-1)
        stu2 = nn.functional.normalize(stu2, p=2, dim=-1)
        tch1 = nn.functional.normalize(tch1, p=2, dim=-1)
        tch2 = nn.functional.normalize(tch2, p=2, dim=-1)
        ...
        tch1 = self.gather_with_grad(tch1)
        tch2 = self.gather_with_grad(tch2)
